...
/*tm изменено чтобы автор мог видеть свои неопубликованные
...
*/
                ...
                if (is_numeric($published)) 
                {
                        if ($tm==1){
                            $userId = $user->get('id');
                            $query->where('a.created_by = ' . (int)$userId .' OR '. $publishedWhere . ' = ' . (int) $published);
                        }
                        else{
                            $query->where($publishedWhere . ' = ' . (int) $published);
                        }
		...
		elseif (is_array($published))
		{
			...
			$published = implode(',', $published);
                    ...
                    if ($tm==1){
                            $userId = $user->get('id');
                            $query->where('a.created_by = ' . (int)$userId . ' OR ' . $publishedWhere . ' IN (' . $published . ')');}
                    else{
                            // Use article state if badcats.id is null, otherwise, force 0 for unpublished
                            $query->where($publishedWhere . ' IN (' . $published . ')');}
		}             
