	...
	public static function edit($article, $params, $attribs = array(), $legacy = false)
	{
		...
		if (property_exists($article, 'checked_out') && property_exists($article, 'checked_out_time') && $article->checked_out > 0 && $article->checked_out != $user->get('id'))
		{
			...
			return '<span class="hasTooltip" title="' . JHtml::tooltipText($tooltip. '', 0) . '">' . $button . '</span>';
                ...
                $scsid=$params->get('num_cat_scarticle');
                $scrid=$params->get('num_cat_screcarticle');
                if ($article->catid==$scsid)
                {
		$url = 'index.php?option=com_content&task=articlesc.edit&a_id=' . $article->id . '&return=' . base64_encode($uri);
                }
                else if ($article->catid==$scrid)
                {
		$url = 'index.php?option=com_content&task=articlescrecens.edit&a_id=' . $article->id . '&return=' . base64_encode($uri);
                }
                else {
